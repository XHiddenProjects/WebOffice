    ...
    if($('#supportTickets-table tbody tr td').length>1){
        $('#supportTickets-table tbody tr').each((_,e)=>{
            ...
            results = results.map(r=>{
                r.assigned_to = !r.assigned_to||JSON.parse(r.assigned_to).length==0 ? '' : r.assigned_to;
                r.attachments = !r.attachments||JSON.parse(r.attachments).length==0 ? '' : r.attachments;
                return r;
            });
        ...
        const location = navigator.geolocation;
            ...
            function showPosition(position) {
                ...
                fetch(`https://api.weather.gov/points/${lat},${lon}`)
                    ...
                    city = data.properties.relativeLocation.properties.city;
                    state = data.properties.relativeLocation.properties.state;
                    $('#deviceLocation').val(`${city}, ${state}`);
                });
        }
    }
    $('.randomAssetTag').each((_,e)=>{
        $(e).on('click',function(){
            const assetTag = 'ABCDEFGHIJKLMNOPQRSTUVWXYZ0123456789'.split('');
            let randAssetTag='';
            for(let i=0;i<6;i++){
                const rand = Math.floor(Math.random() * assetTag.length);
                randAssetTag+=assetTag[rand];
            }
            $(this).parent().find('input[type="text"]').val(randAssetTag);
        });
    });
});